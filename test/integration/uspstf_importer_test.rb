...
class UspstfImporterTest < ActiveSupport::TestCase
  test 'import USPSTF data dump into the database' do
    ...
    artifacts = Artifact.where('remote_identifier = ?', "#{Repository::USPSTF}_SR_358")
    ...
    artifacts = Artifact.where('remote_identifier = ?', "#{Repository::USPSTF}_GR_38")
    assert_equal(1, artifacts.count)
    artifact = artifacts.first
    ...
    # Check example tool
    artifacts = Artifact.where('remote_identifier = ?', "#{Repository::USPSTF}_TOOL_248")
    assert_equal(1, artifacts.count)
    artifact = artifacts.first
